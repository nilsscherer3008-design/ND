...
def main():
    ...
    grund_tempo = getattr(voice.config, "length_scale", 1.0) or 1.0
    log(f"Modell {MODELL}: {gesamt} Sprecher, Grundtempo {grund_tempo}")
    ...
    for nr in nummern:
        ...
        try:
            ...
            try:
                ...
                pass
            fertig.append((nr, dauer))
            log(f"  ♪ Sprecher {nr:3} -> {ziel.name} ({ziel.stat().st_size // 1024} KB, {dauer}s)")
    ...
    marke = str(int(time.time()))      # gegen alte Dateien im Zwischenspeicher
    seite = baue_seite(fertig, basis, marke, grund_tempo)
    (PROBEN_SEITE / "index.html").write_text(seite, encoding="utf8")
    (PROBEN_SEITE / "stimmen.json").write_text(
        json.dumps({"modell": MODELL, "grundtempo": grund_tempo,
                    "sprecher": [{"nr": n, "sekunden": d} for n, d in fertig]},
                   ensure_ascii=False, indent=1), encoding="utf8")
    log(f"\nFertig: {len(fertig)} Proben. Seite unter /proben/")
    return 0


def baue_seite(nummern, basis, marke="", grund_tempo=1.0):
    karten = "\n".join(f"""  <div class="p">
    <b>Sprecher {n} <span class="d">{d}s</span></b>
    <audio controls preload="none" src="{basis}probe-{n:03d}.mp3?v={marke}"></audio>
    <label><input type="checkbox" data-n="{n}"> merken</label>
  </div>""" for n, d in nummern)
